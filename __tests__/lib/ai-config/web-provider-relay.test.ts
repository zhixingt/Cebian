...
import type { DomRelayRequest } from '@/lib/ai-config/web-provider-content-script';
...
describe('injectDomRelay (T7: ⑧ ISOLATED then MAIN injection + message contract)', () => {
  beforeEach(() => {
  ...
  it('injects ISOLATED world first, then MAIN world (order matters for bridge availability)', async () => {
    ...
    await injectDomRelay(42, makeRequest());
  ...
  it('MAIN injection receives the dom relay request as args (providerId, modelId, message, domStrategy)', async () => {
    const calls: any[] = [];
    (chrome.scripting.executeScript as any) = vi.fn((opts: any) => {
      calls.push(opts);
      return Promise.resolve([{ result: { ok: true } }]);
    ...
    await injectDomRelay(42, makeRequest({
      providerId: 'kimi',
      modelId: 'kimi-k2',
      message: 'hi from kimi',
    }));
    const main = calls.find(c => c.world === 'MAIN');
    expect(main).toBeDefined();
    expect(main.target).toEqual({ tabId: 42 });
    // Args is the request object (the MAIN-world orchestrator)
    expect(main.args).toEqual([expect.objectContaining({
      providerId: 'kimi',
      modelId: 'kimi-k2',
      message: 'hi from kimi',
      domStrategy: expect.objectContaining({
        input: expect.objectContaining({ selector: expect.any(String) }),
        reader: expect.objectContaining({ assistantMessageSelector: expect.any(String) }),
      }),
    })]);
  });
  ...
  it('throws if MAIN injection fails (e.g., page not ready, CSP blocks)', async () => {
    ...
    (chrome.scripting.executeScript as any) = vi.fn((_opts: any) => {
      call++;
      // 1st call (ISOLATED) ok, 2nd call (MAIN) fails
      if (call === 1) return Promise.resolve([{ result: { ok: true } }]);
      return Promise.reject(new Error('Cannot inject script'));
    });
    await expect(injectDomRelay(42, makeRequest())).rejects.toThrow('Cannot inject script');
  });
});

describe('message type constants (T7: contract for MAIN → ISOLATED → SW)', () => {
  it('exports 5 message type constants with expected string values', () => {
    ...
    expect(WEB_LLM_NEEDS_RELOGIN).toBe('WEB_LLM_NEEDS_RELOGIN');
  });

  it('WebProviderRelayMessage union includes all 5 message shapes', () => {
    // Compile-time check: assignable to the union.
    const ready: WebProviderRelayMessage = { type: WEB_LLM_RELAY_READY, providerId: 'glm' };
    const chunk: WebProviderRelayMessage = { type: WEB_LLM_CHUNK, providerId: 'glm', text: 'x' };
    const done: WebProviderRelayMessage = { type: WEB_LLM_DONE, providerId: 'glm' };
    const err: WebProviderRelayMessage = { type: WEB_LLM_ERROR, providerId: 'glm', error: 'x' };
    const relogin: WebProviderRelayMessage = { type: WEB_LLM_NEEDS_RELOGIN, providerId: 'glm', status: 401, message: 'x' };
    expect([ready, chunk, done, err, relogin]).toHaveLength(5);
  });
});
